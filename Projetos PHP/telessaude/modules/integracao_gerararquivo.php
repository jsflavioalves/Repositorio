...
if(@$_POST['mes'] && @$_POST['ano']){
    ...
    /// erro está aqui no addProfissionalSaude
		$fileStr .= '$profissionalTeleconsultorias->addProfissionalSaude(null, "'.$row['cpf'].'", "'.$row['nome'].'", "'.$row['estabelecimento_codigo_cnes'].'", "'.$row['codigo_cbo'].'", null, "'.$row['tipo_profissional'].'", "'.$row['sexo'].'");';
		$fileStr .= BR;
	}
	$fileStr .= BR.BR;	
	...
	while($row = $queryTeleconsultorias->fetch()){
		...
		$fileStr .= '$teleconsultoria->addSolicitacao("'.$row['data_hora_solicitacao'].'", TipoTeleconsultoria::ASSINCRONA, CanalAcesso::INTERNET, "'.$row['nr_cpf'].'", "'.$row['codigo_cbo'].'", "'.$row['estabelecimento_codigo_cnes'].'", null, "'.$row['tipo_profissional'].'", ["'.$row['cid'].'"], ["'.$row['ciap'].'"], "'.$row['data_hora_resposta'].'", EvitouEncaminhamentoTeleconsultoria::NAO, IntencaoEncaminhamentoTeleconsultoria::NAO, GrauSatisfacao::SATISFEITO, ResolucaoDuvidaTeleconsultoria::ATENDEU_TOTALMENTE, false);';
		$fileStr .= BR;
	}	
	...
	while($row = $queryProfissionaisTelediagnosticos->fetch()){
		$fileStr .= '$profissionalTelediagnosticos->addProfissionalSaude(null, "'.$row['cpf'].'", "'.$row['nome'].'", "'.$row['estabelecimento_codigo_cnes'].'", "'.$row['codigo_cbo'].'", null, "'.$row['tipo_profissional'].'", "'.$row['sexo'].'");';
		$fileStr .= BR;
	}
	$fileStr .= BR.BR;	
	...
	$fileStr .= BR."?>";
	$fp = fopen('integracao/temp2.php', 'w');
	fwrite($fp, $fileStr);
	fclose($fp);	
}
...
?>
	...
	<a href="integracao/temp2.php">BAIXAR...</a>