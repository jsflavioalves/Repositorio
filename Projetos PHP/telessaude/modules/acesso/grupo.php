...
if(isset($_POST['checkdel'])){	
	if($auth->isAdmin()){		
		...
		$$
		begin
		delete from tb_grupo_transacao where cd_grupo in(".implode(',', $_POST['checkdel']).");
		delete from tb_grupo where ci_grupo in(".implode(',', $_POST['checkdel']).");
		end;
		$$";		
		if(@execute($sqlRemove)){
