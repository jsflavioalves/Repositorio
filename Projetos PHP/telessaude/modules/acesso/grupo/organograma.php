...
//Importando classes
Loader::import('br.ufc.nuteds.TipoTransacao');
...
var data = { description: "'.findDescr($queryGrupos->rowCount()).'",
	...
	while($row = $queryGrupos->fetch()){
		...
		if($rowNumProf['count'] > 0){		
			...
			from tb_usuario_grupo tug 
			inner join tb_usuario tu on(tug.cd_usuario=tu.ci_usuario)
			inner join tb_profissional tp on(tug.cd_usuario=tp.ci_profissional)
...
function getTransactionTemplate() {
	...
	var itemTemplate = jQuery(
	  '<div class="bp-item bp-corner-all bt-item-frame">'
		...
		+ '<div class="bp-item" style="top:91px; left:210px; width:200px; height:30px;"><img name="add" src="assets/icon_add.png" style="padding-right:4px;"/><img name="edit" src="assets/icon_edit.png" style="padding-right:4px;"/><img name="del" src="assets/icon_del.png"/></div>'
	+ '</div>'
	).css({
		width: result.itemSize.width + "px",
		height: result.itemSize.height + "px"
	...
	var itemTemplate = jQuery(
	  '<div class="bp-item bp-corner-all bt-item-frame">'
		+ '<div name="titleBackground" class="bp-item bp-corner-all bp-title-frame" style="top: 2px; left: 2px; width: 220px; height: 20px;">'
			...
			+ '</div>'
		+ '</div>'
		+ '<div name="cpf" class="bp-item" style="top: 26px; left: 15px; width: 220px; height: 18px; font-size: 12px;"></div>'
		+ '<div name="localidade" class="bp-item" style="top: 44px; left: 15px; width: 220x; height: 18px; font-size: 12px;"></div>'
		+ '<div name="description" class="bp-item" style="top: 62px; left: 15px; width: 200px; height: 33px; font-size: 10px;"></div>'
	+ '</div>'
	).css({
		width: result.itemSize.width + "px",
		height: result.itemSize.height + "px"
