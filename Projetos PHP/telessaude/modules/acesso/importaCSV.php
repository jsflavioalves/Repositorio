...
?>
	...
	if ($_FILES['pdf']['name'] != "") {
	    $pasta = './csv/';
	 	...
	 	while(!feof($arq))
	 	   ...
	 	   //         echo $linha[0].$linha[1].$linha[2].$linha[3].$linha[4].$linha[5].$linha[6].$linha[7].$linha[8].$linha[9].$linha[10].$linha[11].$linha[12].$linha[13].$linha[14].$linha[15].$linha[16].'</br>';
	 	         ...
	 	         //      $result = query($sql);
	 	                   ...
	 	                   if($consulta2['nr_cpf']=='' and $aprov!=0){
	 	                   $sql2 = "INSERT INTO tb_aluno_aprov (id_curso, nr_cpf,aprov) VALUES ('$id_curso', '$nr_cpf','1')";
	 	                   $result = query($sql2);}
	 	                   if($consulta2['nr_cpf']!='' and $aprov==0){
	 	                       $delete = "DELETE FROM tb_aluno_aprov WHERE nr_cpf='".$nr_cpf."' and id_curso='".$id_curso."'";
	 	                       $result2 = query($delete);
	 	                   }
	 	               }
	 	          }}
	 	        $aprov=0;
	 	...
	 	echo "quantidade de linhas importadas e atualizadas = ".$ll.'</br>';
