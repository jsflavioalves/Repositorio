...
//Consultando os municípios relacionados a este usuário
$sqlMunicipios = "select tl.ci_localidade, tl.ds_localidade, tl.sg_estado
from tb_teleconsulta tt
inner join tb_localidade tl on(tt.cd_localidade=tl.ci_localidade)
where tt.tp_tipo = $type and (tt.cd_profissional_solicitante = ".$user['ci_usuario']." or tt.cd_profissional_especialista = ".$user['ci_usuario'].")
group by 1,2,3
order by 1 asc";
...
$where = '';
if(@$_POST['search1']){ //CNS Paciente
	$term = $_POST['search1'];
	$where .=  "and tp.nr_codigo = '$term' ";
}
if(@$_POST['search2']){ //Nome do Paciente
	$term = "'%".addslashes($_POST['search2'])."%'";
	$where .=  "and tp.nm_paciente ilike $term ";
}
if(@$_POST['search3']){ //Status
	$term = $_POST['search3'];
	$where .=  "and tt.tp_status = $term ";
}
if(@$_POST['search4']){ //Especialidade
	$term = $_POST['search4'];
	$where .=  "and tt.cd_especialidade = $term ";
}
if(@$_POST['search5']){ //Município
	$term = addslashes($_POST['search5']);
	$where .=  "and tt.cd_localidade = $term ";
}
if(@$_POST['search6']){ //Data
...
?>
					...
					<div class="row">
						<div class="col-lg-10 col-lg-offset-1">
							<div class="col-sm-4">
								...
								<input type="tel" id="search1" name="search1" value="<?php echo @$_POST['search1']; ?>" placeholder="Número do CNS" maxlength="15" class="form-control input-mask-numbers"/>
							...
							<div class="col-sm-4">
								<label>Paciente:</label><input type="text" class="form-control" id="search2" name="search2" placeholder="Nome do Paciente" value="<?php echo @$_POST['search2']; ?>">
							</div>
							<div class="col-sm-3">
								<label>Status:</label>
						...
						<div class="col-lg-10 col-lg-offset-1">
							<div class="col-sm-4">
								...
								while($row = $queryEspecialidades->fetch()){
									if(@$_POST['search4'] == $row['ci_especialidade'])
							...
							<div class="col-sm-4">
								...
								while($row = $queryMunicipios->fetch()){
									if(@$_POST['search5'] == $row['ci_localidade'])
							...
							<div class="col-sm-3">
								...
								<div class="input-group date">
								  <input id="search6" name="search6" type="tel" value="<?php echo @$_POST['search6']; ?>" class="form-control input-mask-date"><span class="input-group-addon"><i class="glyphicon glyphicon-calendar"></i></span>
								</div>
							</div>
							<div class="col-sm-1 text-center">
								<button type="submit" id="btAdd" class="btn btn-info" style="margin-top: 19px;"><span class="glyphicon glyphicon-zoom-in"></span> Pesquisar</button>
							</div>
						</div>	
					</div>
				</div>
	...
	<?php } elseif(@$type == Teleconsulta::TIPO_OPNIAO_FORMATIVA){ 
		...
		if($registros < 1){
			Util::alert('Nenhuma teleconsulta encontrada!');
		}
		else{
			?>
