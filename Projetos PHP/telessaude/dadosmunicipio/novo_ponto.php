...
if(@$_GET['db']) {
	...
	$sqlColumns[] = 'cd_dados_municipio';
	$sqlValues[] = $row['codigo'];

	$sql = 'INSERT INTO nuteds.tb_pontos_telessaude ('.implode(', ', $sqlColumns).') values('.implode(', ', $sqlValues).');';
	
	if(execute($sql)) {
		Controller::setInfo('Ponto de Telessaúde', 'Salvo com sucesso!');	
		Controller::redirect('index.php?page=view&codigo='.$row['codigo']);	
	}
...
<form action="<?php echo Util::setLink(array('db=1')) ?>" method="post" id="formInsertEdit" onsubmit="return test();">		
			...
			<div class="row">				
				<div class="col-md-3">
					...
					<div style="margin-top:7px;">
						<label><input type="radio" name="fl_sala_palestra" value="true" <?php echo (@$rowEdit['fl_sala_palestra'] == 'true' ? 'checked="checked"' : ''); ?>/> Sim &nbsp;&nbsp;&nbsp;</label>
						<label><input type="radio" name="fl_sala_palestra" value="false" <?php echo (@$rowEdit['fl_sala_palestra'] == 'false' ? 'checked="checked"' : ''); ?>/>	Não</label>
					</div>
				</div>
				<div class="col-md-3">
					Telecaridologia?<br>
					<div style="margin-top:7px;">
						<label><input type="radio" name="fl_telecardio" value="true" <?php echo (@$rowEdit['fl_telecardio'] == 'true' ? 'checked="checked"' : ''); ?>/> Sim &nbsp;&nbsp;&nbsp;</label>
						<label><input type="radio" name="fl_telecardio" value="false" <?php echo (@$rowEdit['fl_telecardio'] == 'false' ? 'checked="checked"' : ''); ?>/>	Não</label>
					</div>
				</div>
				<div class="col-md-3">
					Internet?<br>
					<div style="margin-top:7px;">
						<label><input type="radio" name="fl_internet" value="true" <?php echo (@$rowEdit['fl_internet'] == 'true' ? 'checked="checked"' : ''); ?>/> Sim &nbsp;&nbsp;&nbsp;</label>
						<label><input type="radio" name="fl_internet" value="false" <?php echo (@$rowEdit['fl_internet'] == 'false' ? 'checked="checked"' : ''); ?>/>	Não</label>
					</div>
				</div>
				<div class="col-md-3">
					Velocidade Internet
