...
final class Util{
	 ...
	 */
	public static function ireport($reportName, $relName, $params=array(), $popup=false, $type='PDF'){		
	...
	/*public static function reportJSXLS($fileName, $table) {
		echo "<script> 
			var uri = 'data:text/csv;charset=utf8,';
			var table = `{$table}`;
	...
	public static function reportCSV($fileName, $params) {
		$params['filename'] = $fileName;
		$params_serealized = base64_encode(serialize($params));
		echo '<script type="text/javascript">
			var janela = window.open("reports/'.$fileName.'.php?params='. $params_serealized . '", "_blank");
			janela.focus();
		</script>';
	}

	public static function downCSV($fileName, $table) {
		$encoded_csv = mb_convert_encoding($table, 'UTF-16LE', 'UTF-8');
	...
	public static function pagination1($registros, $formSearch=1, $limitPagina=10, $faixa=4, $dif=5){
		$p = (@$_GET['p'] ? $_GET['p'] : 1);		
		$paginas = ceil($registros / $limitPagina);
		$dif = $p - ($faixa / 2);
		$min = 1;
		...
		if($registros > 0){		
			...
			<form id="formPaginacao" name="formPaginacao" method="post">
			<div class="pagination_container text-center">
				<ul class="pagination" style="margin: 0px 0; font-size:11px;">
					<li><a href="javascript:void(0);" onclick="setPaginacao(\''.Util::setLink(array('p='.$back)).'\')">&laquo;</a></li>';
		...
		if($registros != 0){
			$str .= '<div class="col-md-4"><div class="pull-right"><i>'.$descrRows.'</i></div></div>';
		}
		$str .= '</div>';
		return $str;		
	}
