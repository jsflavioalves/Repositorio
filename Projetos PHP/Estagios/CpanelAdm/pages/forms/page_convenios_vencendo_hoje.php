...
<div class="content-wrapper">
    ...
    <div class="content">
      <div class="row">
        ...
        $sql = mysql_query("SELECT * FROM termo_convenio ORDER BY CD_CONVENIO");
          ...
          echo'<br>
                <div class="box box-primary">
                  <div class="box-header">
                    <h3 class="box-title">Convênios vencidos hoje</h3>
                    <a href="convenios_vencidos_opc.php"><button type="submit" class="btn btn-success pull-right">VOLTAR</button></a>
                  </div>';
            ...
            $CD_CONVENIO = $noticias['CD_CONVENIO'];
            $CD_EMPRESA = $noticias['CD_EMPRESA'];
            $dt_fim = $noticias['dt_fim'];


            $sql2 = mysql_query("SELECT * FROM empresas WHERE CD_EMPRESA LIKE '$CD_EMPRESA'");
            $noticias2 = mysql_fetch_array($sql2);
            $nome = $noticias2['nome'];
            $email = $noticias2['email'];
            $representante = $noticias2['representante'];

            $partes = explode("-", $dt_fim);

                    $dia_dt = $partes[0];       
                    $mes_dt = $partes[1];   
                    $ano_dt = $partes[2];               
                    $total = $dia_atual - $dia_dt;

                    if($dia_dt == $dia_atual AND $mes_dt == $mes_atual AND $ano_dt == $ano_atual){
                      echo utf8_encode('<tr>
                        <td>'.$noticias['CD_CONVENIO'].'</td>
                        <td>'.$noticias2['nome'].'</td>
                        <td>'.$noticias['NR_PROCESSO'].'</td>
                        <td>'.$noticias['dt_inicio'].'</td>
                        <td>'.$noticias['dt_fim'].'</td>
                        <td><a href="https://www.gmail.com" target="_blank">'.$noticias2['email'].'</a></td>
                        <td>'.$noticias2['representante'].'</td>
                      </tr>');
                    } 
            }
                  echo '</table>
