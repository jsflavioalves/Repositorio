...
if ($agt_int!=null) {
echo '<a onclick="siim2()" id="nome"><div class="alert alert-success" style="cursor:pointer; margin-top:5px;">' . $nome . '  CNPJ:  '.$CNPJ.' - TEM REGISTRO NO CADASTRO DE EMPRESAS MAS O CONVÊNIO É POR AGENTE: '.$agt_int.'
  </div></a>';
                    }
    ...
    //VERIFICA SE O PODE FAZER OUTRO TERMO
        if ($ano_final != '' and $ano > $ano_final and $agt_int=='') {
          echo utf8_encode('<a onclick="siim2()" id="nome"><div class="alert alert-danger" style="cursor:pointer; margin-top:5px;">' .$nome. '  CNPJ:  '.$CNPJ.' - ACABOU O CONVENIO EM '.$data_fim.'</div></a>');
        ...
        if ($ano < $ano_final) {
    echo utf8_encode('<a onclick="siim2()" id="nome"><div class="alert alert-success" style="cursor:pointer; margin-top:5px;">' .$nome. '  CNPJ:  '.$CNPJ.' - TEM CONVENIO DE '. $dt_inicio.' ATE '.$data_fim.'</div></a>');       
        } 
...
echo utf8_encode('<a onclick="siim2()" id="nome"><div class="alert alert-success" style="cursor:pointer; margin-top:5px;">' .$nome. '  CNPJ:  '.$CNPJ.' - TEM CONVENIO DE '. $dt_inicio.' ATE '.$data_fim.'</div></a>');        
      ...
      echo utf8_encode('<a onclick="siim2()" id="nome"><div class="alert alert-danger" style="cursor:pointer; margin-top:5px;">' .$nome. '  CNPJ:  '.$CNPJ.' - NAO TEM REGISTRO DE CONVENIO.</div></a>');
        } 
