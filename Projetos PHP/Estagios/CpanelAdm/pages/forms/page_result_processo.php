...
<head>
  ...
  <script type="text/javascript">
    ...
    function completar_busca_conv() {
          
        
            $(document).ready(function(){
              $('#nome').autocomplete(
              { 
                source: "search_busca_conv.php",
                minLength: 1,
                delay: 0
              });
        ...
        function completar_busca_conv_atu() {
          
        
            $(document).ready(function(){
              $('#nome1').autocomplete(
              { 
                source: "search_busca_conv_atu.php",
                minLength: 1,
                delay: 0
              });
            ...
            <section class="content">
              ...
              <div class="box box-primary">
                <div class="form-group">
                  ...
                  <div class="box-body">
                    <h3 class="box-title">Atualizar Processo de Convênio</h3>
...
// Recebe o valor enviado
 if(isset($_POST['btna1'])){
$valor = utf8_decode($_POST['nome1']);
...
if($resulatdo != 0){
  ...
  $sql2 = mysql_query("SELECT * FROM termo_convenio WHERE CD_EMPRESA LIKE '$noticias2->CD_EMPRESA'");
  $res = mysql_num_rows($sql2);

  if($res != 0 ){
  $noticias = @mysql_fetch_object($sql2);

  echo '<form action="acao_atualizar_empresa_convenio.php" enctype="multipart/form-data" method="POST">
  ...
  echo '      <br>
              <table class="table table-hover">
                <tr>
                 ...
                 $sql2 = mysql_query("SELECT * FROM termo_convenio WHERE CD_EMPRESA LIKE '$noticias2->CD_EMPRESA' order by NR_PROCESSO ASC ");
                while($noticias1 = mysql_fetch_array($sql2)){
                      ...
                      echo utf8_encode('  <tr>
                                ...
                                <td><input type="text" class="form-control" value="'.$noticias1["tipo_convenio"].'" disabled></td>');
          ...
          echo '</table>
                <table class="table table-hover">
                  <tr>
                    <h4>N° do Processo a ser Atualizado</h4>';?>
                    <select class="form-control select2" name="processo_atu" style="width: 100%;" required>
                    <?php 
                      require('../../../conn.php');

                      $consulta = mysql_query("SELECT * FROM termo_convenio WHERE CD_EMPRESA LIKE '$noticias2->CD_EMPRESA' order by NR_PROCESSO ASC ");
                      echo $result = mysql_num_rows($consulta);

                      while ($sql = mysql_fetch_array($consulta)) {
                        $NR_PROCESSO = $sql['NR_PROCESSO'];

                        echo utf8_encode('<option>'.$NR_PROCESSO.'</option>');
                      }
                     ?>
                  </select>
<?php
                $sql3 = mysql_query("SELECT * FROM termo_convenio WHERE CD_EMPRESA LIKE '$noticias2->CD_EMPRESA' order by NR_PROCESSO ASC ");
                $array = mysql_fetch_array($sql3);
      echo '    </tr>
                </table>
                <table class="table table-hover">
                  <tr>
                    <th>Novo Nº do Processo</th>
                    <th>Nova Data de Inicio</th>
                    <th>Nova Data Fim</th>
                    <th>Tipo de Convênio</th>
                  <th></th>
                  </tr>
                  <tr>
                    ...
                    <td><input type="text" id="mask3" onkeypress="processo3(mask3)" maxlength="12" class="form-control" placeholder="N° Processo" name="n_processo" value="'.$array["NR_PROCESSO"].'" required></td>
                    <td><input type="text" id="mascara" onkeypress="inserir_conv(mascara)" maxlength="10" class="form-control" placeholder="Data de Início" name="dt_inicio" value="'.$array["dt_inicio"].'" required></td>
                    <td><input type="text" id="mascara1" onkeypress="inserir_conv1(mascara1)" maxlength="10" class="form-control" placeholder="Data Fim" name="dt_fim" value="'.$array["dt_fim"].'" required></td>
                    ...
                    <input type="hidden" class="form-control" name="id_convenio" value="'.$noticias1->CD_CONVENIO.'">
                  </tr>
                </table>
...
if ($res == 0) {
    echo utf8_encode('
       ...
       echo '  <div class="note2 note-danger2"><h4 class="box-heading2"><Strong><font color="red">AVISO! </font></Strong></h4>
               <p><font color="red">Nenhum Convênio Registrado.</font></p>
               <a href="cadastro_empresas_convenio.php"><button type="button" class="btn btn-danger pull-right">Cancelar</button></a>
               </div></div><br>';
  } 
}if ($resultado == 0) {
