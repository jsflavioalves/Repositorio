...
<head>
  ...
  <title>Agência de Estágio | Centros</title>
  ...
  <script type="text/javascript">
    ...
    function mostrar() {
      var mudar = document.getElementById("nome_curso").style.display = 'none';
      var mudar = document.getElementById("busca2").style.display = 'none';
      var mudar = document.getElementById("divAtualiza").style.display = 'block';
      var mudar = document.getElementById("btn10").style.display = 'none';

    }
  function completarprofessor() {
          
        
            $(document).ready(function(){
              $('#prof').autocomplete(
              { 
                source: "search_professor.php",
                minLength: 1,
                delay: 0
              });
    ...
    <div class="content">
        ...
        <div class="col-md-12">
          <div class="box box-primary">
            <div class="box-header">
              <h3 class="box-title">PROFESSORES ORIENTADORES</h3>
              <div class="input-group input-group-sm" style="width: 150px; float:right;">
                <input id="inputName1" onclick="divConv_and();" onkeyup="buscarNoticias_Prof(this.value)" type="text" placeholder="PROCURAR" class="form-control" />
            ...
            <!-- /.box-header -->
            <!--mostra uma tabela com os dados dos professores cadastrados, podendo pesquisar algum específicamente e também a possibilidade de atualizar as informações do professor-->
            <div class="box-body table-responsive no-padding" id="tabela" style="display: block;" >
              <table class="table table-hover">
                ...
                <?php 
                  ...
                  $consulta = mysql_query("SELECT * FROM professor_orientador ORDER BY id_professor DESC LIMIT 7");
                  $td=mysql_num_rows($consulta);

                  if ($td!=0) {
                    while ($sql = mysql_fetch_array($consulta)) {
                      $id_professor = $sql['id_professor'];
                      $nome = $sql['nome'];
                      $siape = $sql['siape'];
                      $telefone = $sql['fone'];
                      $email = $sql['email'];
                      ...
                      echo utf8_encode('<tr>
                              ...
                              <td>'.$siape.'</td>
                              <td>'.$telefone.'</td>
                              <td><a href="https://www.gmail.com" target="_blank">'.$email.'</a></td>
                              <td>'.$lotacao.'</td>
                              <td>'.$cpf.'</td>
                  ...
                  if ($td==0) {
                    echo' <tr>
                            <td>Não Existe Professor(a) cadastrado</td>
                          </tr>';
                  }
                 ?>
              </table>
      ...
      <div class="box box-primary">
        <div class="box-header with-border">
          <h3 class="box-title">Atualizar Professor(a)</h3>
        </div>
        <!--PARTE QUE ATUALIZA UM PROFESSOR POR MEIO DE PESQUISA AUTOCOMPLETE E FORMULÁRIO-->
        <div class="box-body">
          <div class="row">
            <div class="col-md-12">
                <form action="page_atualizar_professor.php" method="POST">
                  <div class="form-group">
                    <input id="prof" name="prof" onkeypress="completarprofessor();" type="text" style="text-transform:uppercase" placeholder="Nome/Siape" class="form-control" /><br>
                    <div class="col-md-12">
                      <button type="submit" class="btn btn-primary pull-right" name="siapebtn" style="float: right; display:block;">Consultar</button>
                    </div>
                  </div>
                </form>
              </div>
      ...
      <div class="col-md-6">
        <div class="box box-primary">
        <div class="box-header with-border">
          <h3 class="box-title">Excluir Professor(a)</h3>
        </div>
        <!--PARTE QUE EXCLUI UM PROFESSOR-->
        <div class="box-body">
          <div class="row">
            <div class="col-md-12">
                <form action="acao_deleta_professor.php" method="POST">
                <select class="form-control select2" name="id_professor" style="width: 100%;">
                  <?php 
                    require('../../../conn.php');

                    $consulta = mysql_query("SELECT * FROM professor_orientador");
                    ...
                    while ($sql = mysql_fetch_array($consulta)) {
                      ...
                      echo utf8_encode('<option value="'.$id_professor.'">'.$nome.' - '.$siape.'</option>');
                    }
                   ?>
                </select><br><br>
                 <div class="col-md-12">
                      <button type="submit" class="btn btn-danger pull-right" name="siapebtn" style="float: right; display:block;">Excluir</button>
                    </div>
                </form>
                </div>
              </div>
...
<script src="../../dist/js/funcs_centros.js"></script>
<script src="../../dist/js/funcs_Prof.js"></script>
