...
<div class="content-wrapper">
    ...
    <div class="content">
      ...
      <div class="row">
        ...
        $sql = mysql_query("SELECT * FROM usuarios_agencia WHERE tipo_usuario='professor' AND status='Pendente'");
        //$noticias = mysql_fetch_array($sql);
        $resultado = mysql_num_rows($sql);


        if ($resultado == 0){
          echo '<div class="alert alert-danger alert-dismissable">NENHUMA PENDÊNCIA!</div>';
        } else if($resultado != 0){

          echo'<br>
                <div class="box box-primary">
                  <div class="box-header">
                    <h3 class="box-title">Professores Pendentes</h3>
                  </div>';
             ...
             echo'<div class="box-body table-responsive no-padding">
                    <table class="table table-hover">
                      <tr>
                        ...
                        <th>TELEFONE</th>
                        <th>LOTAÇÃO</th>
                        <th>ENVIAR E-MAIl DE AUTORIZAÇÃO</th> 
                        <th>STATUS</th>
                        <th></th>
                      </tr>';

            while ($noticias = mysql_fetch_array($sql)) {
            $siape = $noticias['login'];
            $sql2 = mysql_query("SELECT * FROM professor_orientador WHERE siape LIKE '$siape'");
            $noticias2 = mysql_fetch_array($sql2);
            echo utf8_encode('<form method="POST">
                      <tr>
                        <td style="text-align: center;">'.$noticias['id_user'].'</td>
                        <td>'.$noticias['nome'].'</td>
                        <td>'.$noticias['login'].'</td>
                        <td>'.$noticias2['fone'].'</td>
                        <td>'.$noticias2['lotacao'].'</td>
                        <td><a href="https://www.gmail.com" target="_blank">'.$noticias2['email'].'</a></td>
                        <td><label style="color:#FF0000;">'.$noticias['status'].'</label></td>
                        <td><input type="submit" name="btn" class="btn btn-success" value="AUTORIZAR"></td>
                        <td><input type="submit" name="btnexc" style="margin-right: 50px;" class="btn btn-success pull-right" value="EXCLUIR"></td>

                        <input type="hidden" name="nome" value="'.$noticias['nome'].'">
                        <input type="hidden" name="id_user" value="'.$noticias['id_user'].'">
                      </tr></form>');
            }
               echo '</table>
                  </div>
        ...
        mysql_select_db('estagios');
          ...
          if(isset($_POST['btn'])){
            ...
            $up = mysql_query("UPDATE usuarios_agencia SET status='Autorizado' WHERE id_user LIKE '$id_user'");
            echo '<script> alert("Professor(a) '.$nome.' Autorizado(a) com Sucesso!"); </script>';
            echo '<script> window.location="permissoes_professores.php" </script>';
          }
        ?>
        <?php
        require ('../../../conn.php');
        mysql_select_db('estagios');
          ...
          if(isset($_POST['btnexc'])){
            $id_user = $_POST['id_user']; 
            $up = mysql_query("DELETE FROM usuarios_agencia WHERE id_user LIKE '$id_user'"); 
            if ($up!=0) echo '<script> alert("Deletado com Sucesso!"); </script>';
            echo '<script> window.location="permissoes_professores.php" </script>'; 
          }
        ?>
