...
<head>
  ...
  <title>Agência de Estágios UFC | Aditivo Ao Termo de Compromisso</title>
...
<div class="row">
        ...
        <?php 
            ...
            $consulta2 = mysql_query("SELECT * FROM termo_compromisso WHERE matricula_aluno LIKE '$matricula' and obs = ''");
            $resultado = mysql_num_rows($consulta2);

            $sql3 = @mysql_fetch_array($consulta2);
            $id_aluno_pes = $sql3['id_aluno'];
            $matricula_aluno_pes = $sql3['matricula_aluno'];
            $tipo_termo_pes = $sql3['tipo_termo'];
            $id_empresa_pes = $sql3['nome'];
            $prof_orientador_pes = $sql3['prof_orientador'];
            $dt_inicio_pes = $sql3['dt_inicio'];
            $dt_fim_pes = $sql3['dt_fim'];
            $valor_pes = $sql3['valor'];
            $carga_horaria_pes = $sql3['carga_horaria'];
            $rescisao_pes = $sql3['rescisao'];
            $hora_inicio_pes = $sql3['hora_inicio'];
            $hora_fim_pes = $sql3['hora_fim'];
            $variavel_pes = $sql3['variavel'];
            $id_agente_pes = $sql3['id_agente'];
            $id_termo_compromisso_pes = $sql3['id_termo_compromisso'];
          ?>
      <!-- Small boxes (Stat box) -->
      <div class="row">
        <div class="col-md-12">
         ...
         require('../../CpanelAluno/pages/conn.php');
          ...
          $consulta = mysql_query("SELECT * FROM termo_compromisso WHERE matricula_aluno LIKE '$matricula' and obs = ''");
          $resultado=mysql_num_rows($consulta);

            if ($resultado != 0) {
                echo '<div class="callout callout-info">
            <h4>TODOS OS SEUS TERMOS DE COMPROMISSO OBRIGATÓRIO</h4>
          </div>
          <div class="box box-primary">
            <div class="box-header">
              <h3 class="box-title">TERMOS DE COMPROMISSO DE ESTÁGIO</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover">
                <th>ID_TCE</th>
                  ...
                  $consulta3 = mysql_query("SELECT * FROM empresas WHERE CD_EMPRESA LIKE '$id_empresa_pes'");
                  $sql3 = mysql_fetch_array($consulta3);
                  $empresa3 = $sql3['nome'];

                  while($noticias3=@mysql_fetch_array($consulta)){ 
                  ...
                  echo utf8_encode('<tr>');
                  echo utf8_encode('<td>'.$noticias3['id_termo_compromisso'].'</td>
                                    ...
                                    <td>'.$noticias3['rescisao'].'</td>
                                    <td>'.$noticias3['hora_inicio'].'</td>
                                    <td>'.$noticias3['hora_fim'].'</td>
                                    <td><label id="ret_proc_nome" style="color:#00a258;">'.$noticias3['variavel'].'</label></td>
                                    <td>'.$noticias3['carga_horaria'].'</td>
                                    <td>'.$noticias3['tipo_termo'].'</td>
                                    <td>'.$noticias3['classificacao_termo'].'</td>
                                    <td><label id="ret_proc_nome" style="color:#00a258;">'.$noticias3['relatorio'].'</label></td>
                                    <td>'.$noticias3['agente'].'</td>
                                    <td>'.$noticias3['prof_orientador'].'</td> 
                                    <td>'.$noticias3['status'].'</td>');
                  echo utf8_encode('</tr>');
                  }
              echo'</table>
            </div>
            ...
            if ($resultado == 0) {
                echo '<div class="alert alert-danger alert-dismissable">
                    <strong>ATENÇÃO!</strong> VOCÊ NÃO TEM NENHUM TERMO QUE POSSA SER ADITIVADO.</div>';}
         ?>
        </div>
      </div>
      <!-- /.row (main row) -->
...
<script src="../../CpanelAluno/dist/js/funcs_ta.js"></script>
